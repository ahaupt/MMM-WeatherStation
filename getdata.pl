...
$output = {
    ...
    'outdoor-pm2.5-min'		=> $minmax->{'outdoor-pm2.5-min'},
    'outdoor-pm2.5-max'		=> $minmax->{'outdoor-pm2.5-max'},
    'outdoor-pm10'		=> $data->{'outdoor-pm10'},
    'outdoor-pm10-min'		=> $minmax->{'outdoor-pm10-min'},
    'outdoor-pm10-max'		=> $minmax->{'outdoor-pm10-max'},
...
sub sqlite_db {
    my $schema = "indoor_temp REAL, indoor_humi REAL, outdoor_temp REAL, outdoor_humi REAL, pressure REAL, co2 REAL, pm25 REAL, pm10 REAL, time DATETIME DEFAULT CURRENT_TIMESTAMP";
...
sub safe_data {
    my ($dbh, $data) = @_;
    my $sth = $dbh->prepare("INSERT INTO $TABLE_NAME (indoor_temp, indoor_humi, outdoor_temp, outdoor_humi, pressure, co2, pm25, pm10) VALUES (?,?,?,?,?,?,?,?)");
    $sth->execute(
...
#		print qq(SELECT $kind(${inout}_${sensor}) FROM $TABLE_NAME WHERE $clauses->{"${kind}_$t"}), "\n";
    ...
    foreach my $sensor ( qw(indoor-co2 outdoor-pm2.5 outdoor-pm10) ) {
	$sensor =~ m|door\-(.+)$|;
	my $short_name = $1; $short_name =~ s/\.//g;

	$sth = $dbh->prepare("SELECT min($short_name) FROM weatherdata WHERE time > datetime(datetime('now', '-1 days'))");
	$sth->execute();
	$row = $sth->fetchrow_arrayref();
	$data->{"$sensor-min"} = $row->[0];
	$sth = $dbh->prepare("SELECT max($short_name) FROM weatherdata WHERE time > datetime(datetime('now', '-1 days'))");
	$sth->execute();
	$row = $sth->fetchrow_arrayref();
	$data->{"$sensor-max"} = $row->[0];
    }
#    $d->dumpValue($data);
    return $data;
}
